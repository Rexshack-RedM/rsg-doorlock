...
Citizen.CreateThread(function()
    ...
    while true do
        Wait(4)
        local isdead = IsEntityDead(chace.ped)
        local cuffed = IsPedCuffed(chace.ped)
        local hogtied = Citizen.InvokeNative(0x3AA24CCC0D451379, chace.ped)
        local lassoed = Citizen.InvokeNative(0x9682F850056C9ADE, chace.ped)
        local playerCoords, letSleep = GetEntityCoords(chace.ped), true
...
AddEventHandler('rsg-doorlock:changedoor', function(doorID, state)
    local pedCoords = GetEntityCoords(chace.ped, true)
    ...
    local x, y, z = table.unpack(GetEntityCoords(chace.ped, true))
    local prop = CreateObject(prop_name, x, y, z + 0.2, true, true, true)
    local boneIndex = GetEntityBoneIndexByName(chace.ped, "SKEL_R_Finger12")

    SetPedDesiredHeading(chace.ped, heading)

    if not IsEntityPlayingAnim(chace.ped, "script_common@jail_cell@unlock@key", "action", 3) then
        ...
        Wait(100)
        TaskPlayAnim(chace.ped, 'script_common@jail_cell@unlock@key', 'action', 8.0, -8.0, 2500, 31, 0, true, 0, false, 0, false)
        RemoveAnimDict("script_common@jail_cell@unlock@key")
        Wait(750)
        AttachEntityToEntity(prop, chace.ped, boneIndex, 0.02, 0.0120, -0.00850, 0.024, -160.0, 200.0, true, true, false, true, 1, true)
        Wait(250)
        TriggerServerEvent('rsg-doorlock:updateState', doorID, state, function(cb) end)
        Wait(1500)
        ClearPedSecondaryTask(chace.ped)
        DeleteObject(prop)
    end
end)
